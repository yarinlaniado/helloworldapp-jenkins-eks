            ...
            steps {
              container('ssh-agent') {
               sshagent(['DEPLOY_AGNET_SSH']) {
                sh 'ssh -o StrictHostKeyChecking=no -tt ec2-35-180-192-36.eu-west-3.compute.amazonaws.com kubectl set image deployment/hello-world-deployment hello-world=yarinlaniado/helloworld-webapp:${VERSION} -n deployment'
                }
